...
function inferDefaultApiBaseUrl() {
  ...
  if (port === "8000") return "";
  const normalizedProtocol = protocol === "http:" || protocol === "https:" ? protocol : "http:";
  return `${normalizedProtocol}//${hostname || "127.0.0.1"}:8000`;
}
...
const API_BASE_URL = (() => {
  ...
  return String(globalBase || metaBase || storageBase || queryBase || inferDefaultApiBaseUrl() || "")
    .trim()
    .replace(/\/+$/, "");
})();
...
function normalizeDomainValue(value) {
  return value === "unified" ? "general" : value;
}
...
function isSyntaxAnswer(message, text) {
  const answerSource = String(message?.answer_source || "").toLowerCase();
  const lookupStatus = String(message?.lookup_status || "").toLowerCase();
  const sourceText = String(text ?? "");

  if (answerSource.includes("syntax")) return true;
  if (answerSource === "deterministic_command_formatter") return true;
  if (answerSource === "deterministic_cli_syntax") return true;
  if (lookupStatus === "cli_syntax") return true;

  return /```(?:text|cli)?[\s\S]*```/i.test(sourceText) && /\bsyntax\b/i.test(sourceText);
}

function renderAssistantMarkdown(text, options = {}) {
  const syntaxAnswer = Boolean(options.syntaxAnswer);
  const source = String(text ?? "");
  const syntaxSnippet = syntaxAnswer ? extractSyntaxSnippet(source) : "";
  const parts = [];
  ...
  while ((match = blockPattern.exec(source)) !== null) {
    ...
    parts.push(`
      <div class="code-block${syntaxAnswer ? " syntax-block" : ""}">
  ...
  if (after) {
    if (syntaxAnswer && syntaxSnippet) {
      const snippetEscaped = escapeHtml(syntaxSnippet).replace(/\n/g, "<br>");
      const intro = escapeHtml(
        source
          .replace(syntaxSnippet, "")
          .replace(/\s+/g, " ")
          .replace(/\s*[:\-]\s*$/, "")
          .trim()
      );
      const introHtml = intro ? `<p class="syntax-intro">${intro}</p>` : "";
      parts.push(`
        <div class="syntax-response">
          ${introHtml}
          <div class="code-block syntax-block">
            <div class="code-block-body">${snippetEscaped}</div>
          </div>
        </div>
      `);
    } else {
      parts.push(
        ...
        <div class="syntax-response">
          <div class="code-block syntax-block">
            <div class="code-block-body">${escapeHtml(after).replace(/\n/g, "<br>")}</div>
          </div>
        </div>
      `
          : renderBulletedText(after)
      );
    }
  } else if (syntaxAnswer && syntaxSnippet) {
    parts.push(`
      <div class="syntax-response">
        <div class="code-block syntax-block">
          <div class="code-block-body">${escapeHtml(syntaxSnippet).replace(/\n/g, "<br>")}</div>
        </div>
      </div>
    `);
  }
...
function extractSyntaxSnippet(text) {
  const source = String(text ?? "").trim();
  if (!source) return "";

  const fencedMatch = source.match(/```(?:text|cli)?\s*([\s\S]*?)```/i);
  if (fencedMatch?.[1]) {
    return fencedMatch[1].trim();
  }

  const backtickMatch = source.match(/`([^`]{3,})`/);
  if (backtickMatch?.[1]) {
    return backtickMatch[1].trim();
  }

  const colonMatch = source.match(/\b(?:is|are)\s*:\s*(.+?)(?:[.?!]\s*$|$)/i);
  if (colonMatch?.[1]) {
    return colonMatch[1].trim();
  }

  const quotedMatch = source.match(/\bsyntax\b.*?\bis\b\s*(.+?)(?:[.?!]\s*$|$)/i);
  if (quotedMatch?.[1]) {
    return quotedMatch[1].trim();
  }

  if (/^\s*(?:syntax|command syntax)\b/i.test(source)) {
    return source.replace(/^\s*(?:syntax|command syntax)\s*[:\-]?\s*/i, "").trim();
  }

  return "";
}

function isSyntaxQuestion(question) {
  const text = String(question ?? "").toLowerCase();
  if (!text) return false;
  return (
    text.includes("syntax") ||
    text.includes("command syntax") ||
    text.includes("what is the command") ||
    text.includes("what does the command")
  );
}
...
function addMessage(role, text, meta = null, debug = null, messageData = null, questionText = "") {
  ...
  if (role === "assistant") {
    const syntaxAnswer = isSyntaxAnswer(messageData, text) || isSyntaxQuestion(questionText);
    if (syntaxAnswer) {
      bubble.classList.add("syntax-answer");
    }
    bubble.innerHTML = renderAssistantMarkdown(text, { syntaxAnswer });
  } else {
...
function applyConversationContext(conversation) {
  ...
  if (typeof conversation.domain === "string") {
    els.domainSelect.value = normalizeDomainValue(conversation.domain) || "auto";
  }
  updateUiState();
}
