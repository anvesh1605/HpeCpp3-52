...
from backend.config import QWEN_MODEL_PATH, RELEASE_LSTM_MODEL_PATH, RELEASE_LOOKUP_INDEX_PATH
...
SYSTEM_PROMPT = (
    ...
    "If the retrieved answer lists types, methods, goals, requirements, or conditions, format them as bullet points.\n"
...
5. For command-purpose questions, explain purpose only if the retrieved answer already includes it.
6. If only syntax was retrieved, say so clearly and do not invent purpose.
7. For release notes, keep the answer factual and concise.
8. Use Markdown formatting when it improves readability.
9. Do not truncate the response.
10. Do not start with "The documented answer is".
11. Return only the formatted answer.
"""

DEFAULT_QWEN_MODEL_PATH = QWEN_MODEL_PATH
DEFAULT_LSTM_MODEL_PATH = RELEASE_LSTM_MODEL_PATH
DEFAULT_LOOKUP_INDEX_PATH = RELEASE_LOOKUP_INDEX_PATH
...
def format_cli_syntax_answer(question: str, lookup_answer: str, metadata: Dict[str, str] | None = None) -> str:
    ...
    if is_command_syntax_question(cleaned_question) and bad_syntax:
        return _clean_markdown_text(
            "I found a related syntax entry, but it appears to contain table-of-contents or OCR artifacts, so I cannot safely return it as exact command syntax."
        )
    ...
    if _is_syntax_only_text(lookup_answer):
        if is_bad_syntax_artifact(lookup_answer, command):
            return _clean_markdown_text(
                "I found a related syntax entry, but it appears to contain table-of-contents or OCR artifacts, so I cannot safely return it as exact command syntax."
            )
...
def validate_qwen_answer(
    ...
    family = normalize_whitespace(data_family).lower()
    min_ratio = 0.4 if family in {"product", "product_documentation", "product docs", "product_docs"} and predicted_intent == "concept_explanation" else 0.7
