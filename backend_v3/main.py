...
class ChatRequest(BaseModel):
    ...
    domain: str = Field(default="auto", description="auto, release, product, or general")
    selected_switch: str = ""
...
service: Optional[AnswerService] = None
service_lock = Lock()
...
def chat(request: ChatRequest) -> dict:
    ...
    if conversation_id:
        ...
        if stored_session is not None:
            current_service.sessions[conversation_id] = stored_session
    else:
        conversation_id = current_service.new_session_id()

    result = current_service.chat(
    ...
    conversation_summary = conversation_store.append_turn(
        ...
        question=request.question,
        result=result,
        session_state=current_service.sessions.get(conversation_id),
