...
def _product_question_profile(question: str, slots: Dict[str, str], predicted_intent: str) -> Dict[str, object]:
    ...
    if not profile_slots.get("topic"):
        topic = _product_topic_from_question(question)
        if topic:
            profile_slots["topic"] = topic
...
def _product_answer_looks_fragmented(answer: str, question_type: str = "") -> bool:
    text = _clean(answer)
    if not text:
        return True
    ...
    if question_type == "concept_explanation":
        ...
        if len(text.split()) <= 4:
            return True
        if re.match(r"^(?:key goals of [a-z0-9_ -]+ include|high availability: key goals of ha include)$", lowered):
            return True
    return False
...
def _llm_understand_question(
    ...
    prompt = (
        ...
        "{\n"
        '  "domain": "product|release|general",\n'
        '  "intent": "string",\n'
...
GENERAL_SYSTEM_PROMPT = (
    "You are a helpful general-purpose assistant for the HPE Aruba QA application.\n"
    "Answer directly using your own knowledge and the conversation context.\n"
    "Do not rely on the local documentation lookup data or mention retrieval.\n"
    "If the user asks a follow-up, use the prior conversation context.\n"
    "If the question is ambiguous, ask one short clarifying question.\n"
)

GENERAL_USER_PROMPT_TEMPLATE = """Conversation context:
Previous question:
{previous_question}
...
Do not mention lookup data, data folders, or retrieval.
Return only the answer.
"""


GENERAL_FALLBACK_RESPONSE = "I'm unable to generate a general answer right now because the model is not available."
...
def _build_general_prompt(
    ...
    previous_answer = _clean(session_context.get("last_final_answer", "")) or _clean(session_context.get("last_lookup_answer", ""))
    context_payload = {
        "switch": _clean(selected_context.get("switch", "")),
        "version": _clean(selected_context.get("version", "")),
        "sub_version": _clean(selected_context.get("sub_version", "")),
        "domain": _clean(selected_context.get("domain", "")),
    }
    return GENERAL_USER_PROMPT_TEMPLATE.format(
        previous_question=previous_question or "NONE",
        previous_answer=previous_answer or "NONE",
        selected_context_json=json.dumps(context_payload, ensure_ascii=False, sort_keys=True),
...
def answer_general_question(
    ...
    prompt = _build_general_prompt(cleaned_question, session_context, selected_context)
    qwen_loaded = (
        qwen_bundle is not None
    ...
    if not qwen_loaded:
        final_answer = GENERAL_FALLBACK_RESPONSE
        ...
        return {
            ...
            "raw_lstm_intent": "general",
            "predicted_intent": "general",
            "final_intent": "general",
            "slots": {},
            "lookup_status": "general",
            "lookup_key_used": None,
            "lookup_answer": None,
            "qwen_used": False,
            "qwen_answer": None,
            ...
            "confidence": 0.0,
            "similarity": 0.0,
            "validation_reason": "general model unavailable",
            "debug": {
    ...
    final_answer = qwen_answer.strip() if qwen_answer and qwen_answer.strip() else GENERAL_FALLBACK_RESPONSE
    qwen_validation_passed = bool(qwen_answer and qwen_answer.strip())
    answer_source = "qwen_general" if qwen_validation_passed else "general_fallback"
    _update_session_context(
        session_context,
        cleaned_question,
        {},
    ...
    return {
        ...
        "source_type": "general",
        "data_family": "general",
        "confidence": 1.0 if qwen_validation_passed else 0.0,
        "similarity": 0.0,
        "validation_reason": "general generation",
        "debug": {
            "general_prompt": prompt,
            "general_model_loaded": True,
        },
...
class ProductRuntime:
    ...
    def answer(
        ...
        deterministic_slots = extract_slots_from_question(cleaned_question)
        merged_question_slots = dict(deterministic_slots)
        merged_question_slots.update(llm_slots)
        slots = _merge_context_slots(merged_question_slots, session_context, selected_context, use_session_context=True)
        ...
        if lookup_status == "found" and _product_answer_looks_fragmented(lookup_answer, profile_question_type):
            grounded = False
...
class AnswerService:
    ...
    def create(cls, device: Optional[torch.device] = None) -> "AnswerService":
        ...
        allow_cpu_qwen = os.getenv("ALLOW_CPU_QWEN", "").strip().lower() in {"1", "true", "yes", "y", "on"}
        if device.type != "cuda" and not allow_cpu_qwen:
            qwen_bundle = QwenBundle(
                requested_path=str(QWEN_MODEL_PATH),
                loaded=False,
                error="Qwen loading is disabled on CPU by default to keep the backend responsive. Set ALLOW_CPU_QWEN=1 to force a CPU load.",
            )
        else:
            try:
    ...
    def _session(self, session_id: str) -> Dict[str, Dict[str, Optional[str]]]:
        if session_id not in self.sessions:
            ...
            if "general" not in self.sessions[session_id] and "unified" in self.sessions[session_id]:
                self.sessions[session_id]["general"] = self.sessions[session_id]["unified"]
            self.sessions[session_id].setdefault("general", _session_template())
    ...
    def resolve_domain(self, requested_domain: str, question: str, session: Optional[Dict[str, Dict[str, Optional[str]]]] = None) -> str:
        ...
        if domain == "unified":
            return "general"
        if domain in {"release", "product", "general"}:
            return domain
        session = session or {}
        if self.qwen.loaded:
            ...
            if llm_domain == "unified":
                return "general"
            if llm_domain in {"release", "product", "general"}:
                return llm_domain
    ...
    def chat(
        ...
        if resolved_domain == "general":
            session_context = session["general"]
    ...
    def health(self) -> Dict[str, object]:
        return {
            ...
            "general_mode_available": self.qwen.loaded,
