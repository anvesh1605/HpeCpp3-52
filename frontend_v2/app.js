...
const els = {
  ...
  switchContextStatus: $("switchContextStatus"),
...
const STRUCTURED_LABEL_HEADS = new Set([
  "feature",
  "known",
  "symptom",
  "scenario",
...
function isStructuredHeading(line) {
  ...
  return STRUCTURED_LABEL_PREFIXES.some((prefix) => normalized === prefix.replace(/:$/, "").toLowerCase());
}

function mergeStructuredLines(lines) {
  const merged = [];
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const currentLower = current.toLowerCase().replace(/:$/, "");
    const nextLower = next.toLowerCase();

    if (currentLower === "feature" && nextLower.startsWith("caveat:")) {
      merged.push(`Feature Caveat: ${next.slice(next.indexOf(":") + 1).trim()}`);
      index += 1;
      continue;
    }
    if (currentLower === "known" && nextLower.startsWith("issue:")) {
      merged.push(`Known Issue: ${next.slice(next.indexOf(":") + 1).trim()}`);
      index += 1;
      continue;
    }
    if (STRUCTURED_LABEL_HEADS.has(currentLower) && next && !isStructuredHeading(next)) {
...
function splitIntoReadablePoints(text) {
  ...
  const protectedLabelPrefixes = STRUCTURED_LABEL_PREFIXES;
  ...
  for (const paragraph of paragraphs) {
    const lines = paragraph
      ...
      .map((line) => line.trim())
      .filter(Boolean);
    const chunks = mergeStructuredLines(lines);

    for (const chunk of chunks) {
      const normalizedChunk = chunk.replace(/\s+/g, " ").trim();
      if (!normalizedChunk) continue;
      const bulletStrippedChunk = normalizedChunk.replace(/^[\s\-*•\d.()]+/, "").trim();
      ...
      if (normalizedChunk.length > 140) {
        if (
          protectedLabelPrefixes.some(
            (prefix) => normalizedChunk.startsWith(prefix) || bulletStrippedChunk.startsWith(prefix)
          )
        ) {
...
function renderBulletedText(text) {
  ...
  if (points.length <= 1) {
    ...
    if (/^[\s\-*\u2022\d.()]+/.test(sourceText)) {
      const singlePoint = normalizeBulletPointSafe(points[0] || sourceText);
      return `
        <ul class="answer-points">
          <li>${renderInlineMarkdown(singlePoint)}</li>
        </ul>
      `;
    }
  ...
  return `
    <ul class="answer-points">
      ${points.map((point) => `<li>${renderInlineMarkdown(normalizeBulletPointSafe(point))}</li>`).join("")}
    </ul>
  `;
}

function normalizeBulletPoint(point) {
  return String(point ?? "")
    .replace(/^[\s\-*•\d.()]+/, "")
    .trim();
}
...
function normalizeBulletPointSafe(point) {
  return String(point ?? "")
    .replace(/^[\s\-*\u2022\d.()]+/, "")
    .trim();
}
...
function updateUiState() {
  ...
  const { switch: sw } = currentContext();
  if (els.switchContextStatus) {
    if (state.switchContextNotice) {
      els.switchContextStatus.textContent = state.switchContextNotice;
    } else if (sw) {
      els.switchContextStatus.textContent = `Using switch context: ${sw}`;
    } else {
      els.switchContextStatus.textContent = "No switch selected yet.";
    }
  }
  renderContextBadges();
  ...
  for (const message of messages) {
    addMessage(
      ...
      message.role === "assistant" && state.showDebug
        ? [
          { label: message.selected_switch ? `switch ${message.selected_switch}` : "switch", kind: "good" },
          { label: message.answer_mode || message.answer_source || "mode", kind: "warn" },
          { label: message.grounded ? "grounded" : "ungrounded", kind: message.grounded ? "good" : "warn" },
        ]
        : null,
...
function debugHtml(result) {
  const slots = result.slots || result.extracted_entities || {};
  const availability = result.debug?.availability_check || {};
  const modelVotes = result.model_votes || result.debug?.model_votes || {};
  const topCandidates = result.top_candidates || result.debug?.top_candidates || [];
  return `
    <div><strong>Conversation:</strong> ${escapeHtml(result.conversation_id || "-")}</div>
    <div><strong>Selected switch:</strong> ${escapeHtml(result.selected_switch || "-")}</div>
    <div><strong>Predicted source:</strong> ${escapeHtml(result.predicted_source || result.domain || "-")}</div>
    <div><strong>Predicted intent:</strong> ${escapeHtml(result.predicted_intent || "-")}</div>
    <div><strong>Answer mode:</strong> ${escapeHtml(result.answer_mode || "-")}</div>
    <div><strong>Grounded:</strong> ${result.grounded ? "yes" : "no"}</div>
    <div><strong>Target value used:</strong> ${result.target_value_used ? "yes" : "no"}</div>
    <div><strong>Lookup status:</strong> ${escapeHtml(result.lookup_status || "-")}</div>
    <div><strong>Lookup context:</strong> ${escapeHtml(result.lookup_context || "-")}</div>
    ...
    <div><strong>Qwen loaded:</strong> ${result.qwen_loaded ? "yes" : "no"}</div>
    <div><strong>Bypass Qwen:</strong> ${result.bypass_qwen ? "yes" : "no"}</div>
    ...
    <div><strong>Model votes:</strong> ${escapeHtml(JSON.stringify(modelVotes, null, 2))}</div>
    <div><strong>Top candidates:</strong> ${escapeHtml(JSON.stringify(topCandidates, null, 2))}</div>
    <div><strong>Availability:</strong> ${escapeHtml(JSON.stringify(availability, null, 2))}</div>
    <div><strong>Matched row:</strong> ${escapeHtml(JSON.stringify(result.matched_dataset_row || null, null, 2))}</div>
  `;
}
...
async function sendQuestion(question) {
  ...
  try {
    ...
    if (result.selected_switch && result.selected_switch !== els.switchSelect.value) {
      els.switchSelect.value = result.selected_switch;
    }
    if (result.debug?.switch_auto_switched && result.selected_switch) {
      state.switchContextNotice = `Auto-switched switch context to ${result.selected_switch} based on the question.`;
    } else {
      state.switchContextNotice = "";
    }
    updateUiState();
    ...
    nextMessages.push({
      ...
      answer_source: result.answer_source || result.predicted_source,
      answer_mode: result.answer_mode,
      grounded: Boolean(result.grounded),
      target_value_used: Boolean(result.target_value_used),
      selected_switch: result.selected_switch || context.switch,
      predicted_source: result.predicted_source || result.domain,
