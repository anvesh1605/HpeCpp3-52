...
| UpgradingVSXfrom10.09orlaterto10.12         |                                                      |               |                |                 |        | 22  |
...
3
AOS-CX10.12VirtualSwitchingExtension(VSX)Guide| (6400,8xxx,9300,10000SwitchSeries)
...
| VSX commands                               |                                                       |          | 109 |
| active-gateway                             |                                                       |          | 109 |
| config-syncdisable                         |                                                       |          | 115 |
| inter-switch-link{<PORT-NUM>|lag<LAG-ID>}  |                                                       |          | 116 |
| inter-switch-linkdead-interval             |                                                       |          | 117 |
| inter-switch-linkhello-interval            |                                                       |          | 118 |
| inter-switch-linkhold-time                 |                                                       |          | 119 |
| inter-switch-linkpeer-detect-interval      |                                                       |          | 120 |
| interfacelagmulti-chassis                  |                                                       |          | 121 |
| ipicmpredirect                             |                                                       |          | 122 |
| keepalivedead-interval                     |                                                       |          | 123 |
| keepalivehello-interval                    |                                                       |          | 124 |
| keepalivepeer                              |                                                       |          | 125 |
| keepaliveudp-port                          |                                                       |          | 126 |
| lacpfallback                               |                                                       |          | 127 |
| linkup-delay-timer                         |                                                       |          | 128 |
| linkup-delay-timerexcludelag-list          |                                                       |          | 130 |
| neighbor<IP-ADDRESS>vsx-sync-exclude       |                                                       |          | 131 |
| role{primary|secondary}                    |                                                       |          | 131 |
| showactive-gateway                         |                                                       |          | 132 |
| showactive-gateway<IFNAME>                 |                                                       |          | 134 |
| showinterface<VLAN-NAME>                   |                                                       |          | 135 |
| showlacpaggregates                         |                                                       |          | 136 |
| showlacpinterfaces                         |                                                       |          | 137 |
| showlacpinterfacesmulti-chassis            |                                                       |          | 140 |
| showrunning-configinterface                |                                                       |          | 142 |
| showrunning-configvsx                      |                                                       |          | 143 |
| showrunning-configvsx-sync                 |                                                       |          | 144 |
| showrunning-configvsx-syncpeer-diff        |                                                       |          | 145 |
| showsysteml2-vlan-mac-mode                 |                                                       |          | 146 |
| showvsxactive-forwarding                   |                                                       |          | 146 |
5
AOS-CX10.12VirtualSwitchingExtension(VSX)Guide| (6400,8xxx,9300,10000SwitchSeries)
...
| vsx-syncudp-forwarder         |           |                        | 217 |
| ----------------------------- | --------- | ---------------------- | --- |
| vsx-syncvrrp                  |           |                        | 218 |
| vsx-syncvsx-global            |           |                        | 219 |
| vsxupdate-software            |           |                        | 220 |
| vsxupdate-softwareboot-bank   |           |                        | 221 |
| Configuration                 | conflict  | finder recommendations | 224 |
| Samplerecommendations         |           |                        | 225 |
| Troubleshooting               |           |                        | 227 |
| ISLisout-of-sync              |           |                        | 227 |
|                               | Solution1 |                        | 227 |
|                               | Solution2 |                        | 227 |
|                               | Solution3 |                        | 228 |
| ISLisinblockingstate          |           |                        | 228 |
| TrafficdroponaVSXLAGinterface |           |                        | 230 |
TrafficlossaftertheISLhasbeenout-of-syncandkeepaliveisdown 231
| Failurescenariosandsplitrecovery |     |     | 231 |
| -------------------------------- | --- | --- | --- |
| Activegatewayisunreachable       |     |     | 232 |
BFDreportsaLAGasdownevenwhenhealthylinksarestillavailable 233
| Support               | and Other          | Resources | 235 |
| --------------------- | ------------------ | --------- | --- |
| AccessingArubaSupport |                    |           | 235 |
| AccessingUpdates      |                    |           | 236 |
|                       | ArubaSupportPortal |           | 236 |
|                       | MyNetworking       |           | 236 |
| WarrantyInformation   |                    |           | 236 |
| RegulatoryInformation |                    |           | 236 |
| DocumentationFeedback |                    |           | 237 |
7
AOS-CX10.12VirtualSwitchingExtension(VSX)Guide| (6400,8xxx,9300,10000SwitchSeries)
...
AOS-CX 10.12 Virtual Switching Extension (VSX) Guide | (6400, 8xxx, 9300, 10000 Switch
...
AOS-CX 10.12 Virtual Switching Extension (VSX) Guide | (6400, 8xxx, 9300, 10000 Switch Series)
...
n If you are upgrading from version 10.06/10.07/10.08, see the Virtual Switching Extension (VSX) Guide for
10.08 for steps on how to upgrade VSX to version 10.08. Then, see the steps in this guide on how to
upgrade VSX from version 10.09.

Upgrading VSX from 10.09 or later to 10.12
...
convergence.
AOS-CX10.12VirtualSwitchingExtension(VSX)Guide| (6400,8xxx,9300,10000Switch
25
Series)
...
Keepalive
Keepalive is a layer 3 interface used to exchange heartbeats between VSX peer switches. The heartbeats
are exchanged using the User Datagram Protocol (UDP) and port 7678 (default). During an ISL failure,
the VSX switches use their keepalive connection to determine if both VSX switches are up and running.
In that case, the secondary switch will shut down VSX-LAG ports. This configuration will prevent a split-
brain situation, which can cause unexpected traffic issues. In a split-brain scenario, VSX switches cannot
synchronize MAC, ARP, and routing tables due to the ISL failure.
...
| --------------- | --- | --- | --- | --- | --- |
switch(config-vsx)#
|     |     | no vsx | inter-switch-link | dead-interval |     |
| --- | --- | ------ | ----------------- | ------------- | --- |
117
AOS-CX10.12VirtualSwitchingExtension(VSX)Guide| (6400,8xxx,9300,10000SwitchSeries)
...
vrf <VRF-NAME>
ShowstheIPv4datapathforaspecifiedVRF.
VSXcommands|154
...
ADDR>/<MASK>,orunique
<IP-ADDR>
SpecifiestherouteinformationforanIPv4address
...
split
vsx-peer
ShowstheoutputfromtheVSXpeerswitch.If
...
Aprimaryswitchhardwarereplacement.
n
...
config-if
6400 Administratorsorlocalusergroupmemberswithexecutionrights
| 8100 | config-lag-if  |     | forthiscommand. |
| ---- | -------------- | --- | --------------- |
| 8320 | config-if-vlan |     |                 |
8325
8360
...
|                     | config-addrgroup-ip   |         |              | memberswithexecutionrightsfor  |
8100
thiscommand.
| 8320 | config-addrgroup-ipv6     |     |     |     |
| ---- | ------------------------- | --- | --- | --- |
| 8325 | config-class-<CLASS-TYPE> |     |     |     |
| 8360 | config-policy             |     |     |     |
config-portgroup
8400
...
| --------------- | --------- | ----- | --- | --- | --- | --- |
switch(config-if)#
|     | vsx-sync | rate-limits |     | vlans qos | access-lists | policies |
| --- | -------- | ----------- | --- | --------- | ------------ | -------- |
...
| --------------- | --------- | --- | --- | --- | --- | --- |
switch(config-lag-if)#
|                        |     | vsx-sync   | policies             | rate-limits | qos vlans | access-lists |
| ---------------------- | --- | ---------- | -------------------- | ----------- | --------- | ------------ |
...
Configurationconflictfinderrecommendations|225
