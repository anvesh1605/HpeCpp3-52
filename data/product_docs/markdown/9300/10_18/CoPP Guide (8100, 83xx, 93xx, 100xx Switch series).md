AOS-CX 10.18.xxxx CoPP Guide (8100, 83xx, 93xx, 100xx Switch
series)
...
AOS-CX 10.18.xxxx CoPP Guide (8100, 83xx, 93xx, 10...
...
•  Set the processing priority in the range 0 to 7 (0 - highest priority and 7 - lowest priority).
...
Up to 32 CoPP policies can be defined, but only one can be active on the switch at a time. A CoPP
policy must always be active on the switch. By default, the switch has a CoPP policy named default
which is automatically applied at first boot. The default CoPP policy is the recommended policy for most
environments. It is advised to consult your HPE support personnel before implementing custom CoPP
policies that significantly deviate from the default policy configured rates.
...
The different classes of traffic that can be individually configured are displayed in the list below.

For the HPE Aruba Networking 8320, 8325, 93xx, 100xx Switch Series:
...
•  bfd: Bidirectional Forwarding Detection (BFD) packets with a destination IP address owned by the

switch.

•  bgp: Border Gateway Protocol packets with a destination IPv4 or IPv6 address address owned by the
...
•  dss: Distributed Services Switch (DSS) packets on 100xx Switch series.
...
•  flow-telemetry: Packets received for flow telemetry such as IP Flow Information Export (IPFIX).

NOTE

The flow-telemetry class is not supported for HPE Aruba Networking 8320
Switch series.
...
The ipfix class is not supported for HPE Aruba Networking 8320, 9300,
10040 Switch series.
...
ip-options: Unicast IPv4 or IPv6 packets including option headers.
...
The mdns class is not supported for HPE Aruba Networking 8320 Switch
series.
...
The secure-learn class is not supported for HPE Aruba Networking 8320,
...
For the HPE Aruba Networking 8100 and 8360 Switch Series:

•  acl-logging: Access Control List logging packets.

•  arp-broadcast: Address Resolution Protocol packets with a broadcast destination MAC address.

•  apr-protect: Address Resolution Protocol packets intercepted and inspected for ARP protection.

•  arp-unicast: Address Resolution Protocol packets with a switch system destination MAC address.

•  bfd-control : Bidirectional Forwarding Detection (BFD) control or Ethernet Operations, Administration,

and Maintenance (EOAM).
...
ckets matching the selected class. Range: 1 to 99999.
...
clear copp-policy statistics 20
...
show copp-policy factory-default 35
...
class will be shown. This command will fail with an alert if the user specifies a class that is not configured for
the actively applied CoPP policy.
...
show copp-policy statistics 39
