| AOS-CX | 10.16 |        | Introduction |       |       | to   |
| ------ | ----- | ------ | ------------ | ----- | ----- | ---- |
|        | the   | Web    | UI           | Guide |       |      |
| 4100i, | 5420, | 6000,  | 6100,        | 6200, | 6300, | 6400 |
|        |       | Switch | Series       |       |       |      |
...
AOS-CX 10.16 Introduction to the Web UI Guide | (4100i, 5420, 6xxx Switch Series)
...
4
| AOS-CX10.16 | IntroductiontotheWebUIGuide| |     |     | (4100i,5420,6xxxSwitchSeries) |     |
| ----------- | ---------------------------- | --- | --- | ----------------------------- | --- |
...
| SupportFilepage                       |                                                      |         |           |          | 67  |
|                                       | Creating,downloading,anddeletingsupportfiles         |         |           |          | 69  |
...
|                                       | Zoominginonthegraph                                  |         |           |          | 81  |
|                                       | Downloadingthegraphasanimageor.csvfile               |         |           |          | 81  |
|                                       | Viewinganalertonthegraph                             |         |           |          | 82  |
HPEArubaNetworkingNetworkAnalyticsEnginescripts,agents,andtroubleshooting
| information           |           |           |     |     | 85  |
| --------------------- | --------- | --------- | --- | --- | --- |
...
By default, the 5420, 6200, 6300, and 6400 switches have the HTTPS server enabled on the mgmt and
default VRF. The 4100i, 6000, and 6100 switches have the HTTPS server enabled on the default VRF. On
all switches, the REST API access mode is set to read-write. You must configure a static IP or use the IP
address on the management port (5420, 6200, 6300, 6400 switches only; the 4100i, 6000, and 6100
...
After upgrading or downgrading the AOS-CX software on the switch, you must clear the browser cache to ensure

proper functionality of the Web UI pages for the switch.

Prerequisites

The 5420 and 6200 switches support two VRFs. One mgmt VRF and one default VRF.
...
1. StartasupportedwebbrowserandentertheIPaddressofthemanagementportintheaddress
bar.UseHTTPS.
Forexample:
| https://192.0.2.5 |     | or  | https://[1001::2] |     |     |     |
| ----------------- | --- | --- | ----------------- | --- | --- | --- |
Youmustuseasupportedbrowser,suchasChrome,Firefox,Edge,orSafari.Fordetailson
supportedbrowserversions,seetheReleaseNotesfortheversionofAOS-CXyouareusing.
...
Shows the Virtual Switching Framework (VSF) configuration and status

information. This link is displayed for the 5420, 6200 and 6300 switches

that support VSF. Not applicable to the 4100i, 6000, or 6100 Switch Series.

Shows the Virtual Switching Extension (VSX) configuration and status
...
the HPE Aruba Neworking Solution Exchange.
...
|     | Pre-Standard                                          | Detect:Enableordisablepre-standarddevicedetection. |     |     |
n
4. ClickChange.
VSF page
...
| Figure1 | SNMPPage |     |
| ------- | -------- | --- |
AOS-CX10.16 IntroductiontotheWebUIGuide|(4100i,5420,6xxxSwitchSeries) 41
...
AOS-CX10.16 IntroductiontotheWebUIGuide|(4100i,5420,6xxxSwitchSeries) 45
...
AOS-CX10.16 IntroductiontotheWebUIGuide|(4100i,5420,6xxxSwitchSeries) 51
...
| ------- | -------------------- | --- | --- |
AOS-CX10.16 IntroductiontotheWebUIGuide|(4100i,5420,6xxxSwitchSeries) 57
...
| ------- | -------------------- | --- | --- | --- |
Status panel
TheStatuspaneldisplaysthestatusofCDP,LLDP,clienttracking,andclienttrackingprobeatthe
switchlevel.Youcaneditandchangetheconfiguration.
| Interfaces | panel |     |     |     |
| ---------- | ----- | --- | --- | --- |
TheInterfacespaneldisplaystheportname,CDPstatus,LLDPtype,clienttrackingstatus,client
trackinginterval,andclienttrackinglimitdetailsforeachactiveinterface.Youcaneditandchangethe
configuration.
| Editing | connected | devices | at the switch | level |
| ------- | --------- | ------- | ------------- | ----- |
Usethisproceduretoviewandedittheconnectionstatusesofdevicesconnectedtothedifferent
interfaces.Enablingordisablingsettingsattheswitchleveldoesnotchangetheconfigurationonthe
interface.
1. Inthenavigationpane,expandConnected Devices,andselectConfiguration.
TheConfigurationpageisdisplayed.
AOS-CXWebUIpages|58
...
AOS-CX10.16 IntroductiontotheWebUIGuide|(4100i,5420,6xxxSwitchSeries) 65

1. Inthenavigationpane,expandSecurity,andselectPKI.
ThePKIpageisdisplayed.
| 2. IntheAssociated |     | Application | Detailspanel,clickEdit. |
| ------------------ | --- | ----------- | ----------------------- |
TheAddingfollowingCertificatedialogboxfortheassociatedapplicationisdisplayed.
3. Selecttherequiredcertificatenamefromthedrop-down.
4. ClickOK.
| Port Security |     | page |     |
| ------------- | --- | ---- | --- |
...
must enable port security using the CLI to apply the port security settings. For more information about
port security commands, see the AOS-CX Command-Line Interface Guide.
...
Support files created before a switch reboot or upgrade will not be accessible to users via HPE Aruba Networking

Central.

Creating, downloading, and deleting support files

You can create support files to capture data about the switch.

The following is applicable to 4100i, 6000, 6100, 8100, 8360, 9300/9300S and 10000 switch series.

n Depending on the switch, you can generate a maximum of 10 support files in a 24 hour period, provided
you are not connected to HPE Aruba Networking Central. However, if you are connected to HPE Aruba

Networking Central, the Create option in Support File window is disabled.

n The switch allows only one support file creation request to be processed at a time. Support file creation
requests from the Web UI fails if a request from the HPE Aruba Networking Central or CLI interface is

already in progress.

n The request progress and status will be displayed in the table.

n The download option is available only after a support file is successfully created.

n Delete the support file to optimize the switch state as it consumes switch memory.

n In the Web UI, during the execution of the show tech command, the standby boot-history of the VSF stack is
not collected. Only the VSF conductor information is collected as there is no password-less authentication
...
n The user is not an administrator.

n https-server REST access mode is configured to read-only.

n The switch is managed from HPE Aruba Networking Central.

n The switch has reached the maximum supported file count.

3. Enter the file name.

The file name can contain 5 to 64 alphanumeric, dash and hyphen characters.

4. Click Create.

The row displays the status percentage of the creation. Depending on the switch, the process can
take up to 15 minutes to complete.

To download a support file that is created on the switch:
...
In the Support File panel, select the support file that you want to download, and click Download.

A download window is displayed.

n To enable the Download button, select the support file which is successfully created.

n You can download only the last created support file.

2. Browse to the destination to save the support file and click Save.

The support file is downloaded to the destination.

To delete a support file that is created on the switch:
...
You can delete support file even if the creation is failed.
...
download or delete a script, create an agent, and access the HPE Aruba Neworking Solution
Exchange to find more scripts.
...
1. Zoominandoutonthegraphbyselectingazoomlevelfromtheoptionsdisplayedatthetopof
thetimeseriesgraph:1hour,1day,10days,30days,90days,180days,1year.Youcanalso
selectCustomtoenteraspecificdateandtimerange.
...
HPE Aruba Networking Network Analytics Engine scripts,
agents, and troubleshooting information

For detailed information about the HPE Aruba Networking Network Analytics Engine and the Analytics
dashboard, see the Network Analytics Engine Guide. This guide includes information about using the Web
UI to do the following:
...
n Access scripts on the HPE Aruba Networking Solutions Exchange.
...
Working with the network analytics features | 85
