...
3
AOS-CX10.09IntroductiontotheWebUIGuide|
...
| Sessionpage                           |                                                      |         |           |          | 44  |
| ConfigMgmtpage                        |                                                      |         |           |          | 46  |
| FirmwareUpdatepage                    |                                                      |         |           |          | 47  |
| Pingpage                              |                                                      |         |           |          | 48  |
| Traceroutepage                        |                                                      |         |           |          | 49  |
| ShowTechpage                          |                                                      |         |           |          | 50  |
| SpanningTreepage                      |                                                      |         |           |          | 50  |
|                                       | Editingthespanningtreesettings                       |         |           |          | 52  |
| ConnectedClientspage                  |                                                      |         |           |          | 52  |
| ConnectedDevicesConfigurationpage     |                                                      |         |           |          | 53  |
|                                       | Editingconnecteddevicesattheswitchlevel              |         |           |          | 54  |
|                                       | Editingconnecteddevicesconfigurationatinterfacelevel |         |           |          | 54  |
| PKIpage                               |                                                      |         |           |          | 55  |
|                                       | AddinganddeletinganESTProfile                        |         |           |          | 56  |
|                                       | ViewinganESTProfile                                  |         |           |          | 57  |
|                                       | AddinganddeletingaTAProfile                          |         |           |          | 57  |
|                                       | EditingaTAProfile                                    |         |           |          | 59  |
|                                       | Addinganddeletingacertificate                        |         |           |          | 59  |
|                                       | Uploadingacertificate                                |         |           |          | 60  |
|                                       | Viewinganddownloadingacertificate                    |         |           |          | 61  |
|                                       | Editingassociatedapplicationdetails                  |         |           |          | 61  |
| PortSecuritypage                      |                                                      |         |           |          | 61  |
|                                       | Editingportsecurity                                  |         |           |          | 62  |
| SupportFilepage                       |                                                      |         |           |          | 63  |
|                                       | Creatinganddeletingsupportfiles                      |         |           |          | 64  |
|                                       | Downloadingasupportfile                              |         |           |          | 65  |
| Finding                               | alert details                                        | using   | the Web   | UI       | 66  |
| Working                               | with the                                             | network | analytics | features | 69  |
| ViewingagentinformationusingtheWebUI  |                                                      |         |           |          | 69  |
| WorkingwithanAnalyticstimeseriesgraph |                                                      |         |           |          | 73  |
|                                       | Customizingdatadisplayedonthegraph                   |         |           |          | 74  |
|                                       | Zoominginonthegraph                                  |         |           |          | 75  |
|                                       | Downloadingthegraphasanimageor.csvfile               |         |           |          | 76  |
|                                       | Viewinganalertonthegraph                             |         |           |          | 76  |
ArubaNetworkAnalyticsEnginescripts,agents,andtroubleshootinginformation 80
| Support               | and Other          | Resources |     |     | 81  |
| --------------------- | ------------------ | --------- | --- | --- | --- |
| AccessingArubaSupport |                    |           |     |     | 81  |
| AccessingUpdates      |                    |           |     |     | 82  |
|                       | ArubaSupportPortal |           |     |     | 82  |
|                       | MyNetworking       |           |     |     | 82  |
| WarrantyInformation   |                    |           |     |     | 82  |
| RegulatoryInformation |                    |           |     |     | 82  |
...
AOS-CX 10.09 Introduction to the Web UI Guide |
...
switches, the REST API access mode is set to read-write. You must configure a static IP or use the IP address
on the management port (6200, 6300, 6400 switches only; the 4100i, 6000, and 6100 switches do not have
a management port) to access the Web UI.
...
Clicking the link displays the Analytics Dashboard. Not applicable to the 6000
...
In the navigation pane, select PoE.

The PoE page is displayed.

2.

In the Interfaces panel, select the port, and click Edit.

The Edit PoE Details dialog box is displayed.

3. Configure the following parameters:

n Priority: The priority level—Critical, High, or Low.

n Class: The PoE class assigned to the port.

n Alloc-by Configuration: Power allocation by usage or class.

n Power Enable: Enable or disable power on the port.

n Pre-Standard Detect: Enable or disable pre-standard device detection.
...
Figure 29 Connected Devices page
...
Figure 34 Alerts panel on Analytics Dashboard
...
Finding alert details using the Web UI | 67
...
This section describes the steps to view agent information using the Web UI, and work with an Analytics time
series graph.
...
Figure 38 Analytics panel on Overview page
...
download or delete a script, create an agent, and access the Aruba Solution Exchange (ASE) to find
more scripts.
...
generated, so users cannot create another agent, thus the CREATE AGENT button is disabled.
These scripts also cannot be disabled from this page so the DELETE button is also disabled.
These scripts and agents can only be deleted from the CLI.
...
Working with the network analytics features | 70
...
navigate options as found with other agents. Finally, watch agents do not support parameters so
the Parameters pane in this window will be empty.
...
Figure 46 Zoom options portion of a graph
...
Figure 49 Graph with alert but not metric for 1/1/3
...
Figure 50 Alert details box with View on Graph button displayed
